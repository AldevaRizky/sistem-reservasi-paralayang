    ...
    <div class="container mx-auto px-4 sm:px-6 lg:px-8 flex justify-between items-center">
        ...
        <div class="hidden md:flex items-center space-x-8">
            <ul class="flex items-center space-x-8 font-medium text-gray-300">
                <li><a href="{{ url('/#hero') }}"
                        class="hover:text-white hover:-translate-y-0.5 transition-all duration-300 block">Beranda</a>
                </li>
                <li><a href="{{ url('/#about') }}"
                        class="hover:text-white hover:-translate-y-0.5 transition-all duration-300 block">Tentang</a>
                </li>
                <li class="relative">
                    <button id="product-menu-button"
                        class="flex items-center gap-1 hover:text-white hover:-translate-y-0.5 transition-all duration-300">
                        <span>Produk</span>
                ...
                <li><a href="{{ url('/#contact') }}"
                        class="hover:text-white hover:-translate-y-0.5 transition-all duration-300 block">Kontak</a>
                </li>
            </ul>
            ...
            <div class="flex items-center space-x-5">
                @guest
                    ...
                    <a href="{{ route('login') }}"
                        class="px-5 py-2 bg-gray-600 text-white font-semibold rounded-lg shadow-sm hover:bg-gray-500 hover:-translate-y-0.5 transition-all duration-300">
                        Login
                    </a>
                    <a href="{{ route('register') }}"
                        class="px-5 py-2 bg-green-600 text-white font-semibold rounded-lg shadow-sm hover:bg-green-500 hover:-translate-y-0.5 transition-all duration-300">
                        Register
                    </a>
                @endguest
                    ...
                    <div class="relative">
                        ...
                        <div id="user-menu-dropdown"
                            ...
                            <form method="POST" action="{{ route('logout') }}">
                                @csrf
                                <button type="submit"
                                    class="w-full text-left block px-4 py-2 text-red-500 hover:bg-red-900/40 hover:text-red-400">
                                    Logout
                                </button>
                            </form>
                        </div>
...
<div id="mobile-menu-container" class="fixed inset-0 z-50 hidden">
    ...
    <div id="mobile-menu-panel"
        ...
        <div class="flex items-center justify-between pb-4 border-b border-gray-700">
            ...
            <button id="close-mobile-menu" class="p-2 rounded-md text-gray-400 hover:bg-white/10 hover:text-white"><svg
        ...
        <div class="mt-6 flex flex-col h-full">
            @auth
                ...
                <a href="/booking"
                    class="w-full text-center mb-6 px-5 py-3 bg-green-600 text-white font-semibold rounded-lg shadow-sm hover:bg-green-500 transition-colors duration-300">Booking
                    Now</a>
            @endauth

            <ul class="flex flex-col space-y-1 text-gray-300">
                <li><a href="{{ url('/#hero') }}"
                        class="block py-3 px-3 rounded-md hover:bg-gray-700 hover:text-white transition-colors duration-300">Beranda</a>
                </li>
                <li><a href="{{ url('/#about') }}"
                        class="block py-3 px-3 rounded-md hover:bg-gray-700 hover:text-white transition-colors duration-300">Tentang</a>
                </li>
                <li>
                    <p class="px-3 pt-3 pb-1 text-xs uppercase text-gray-500 font-bold">Produk</p>
                    <ul class="pl-4">
                        <li><a href="{{ url('/#paragliding-packages') }}"
                                class="block py-2 px-3 rounded-md hover:bg-gray-700 hover:text-white transition-colors duration-300">Paket
                                Paralayang</a></li>
                ...
                <li><a href="{{ url('/#contact') }}"
                        class="block py-3 px-3 rounded-md hover:bg-gray-700 hover:text-white transition-colors duration-300">Kontak</a>
                </li>
            </ul>
            ...
            <div class="mt-auto">
                @guest
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <a href="{{ route('login') }}"
                            class="w-full text-center py-3 px-3 rounded-md text-white bg-gray-700 hover:bg-gray-600 transition-colors duration-300">Login</a>
                        <a href="{{ route('register') }}"
                            class="w-full text-center py-3 px-3 rounded-md text-white bg-green-600 hover:bg-green-500 transition-colors duration-300">Register</a>
                    </div>
                @endguest
                @auth
                    ...
                    <form method="POST" action="{{ route('logout') }}">
                        @csrf
                        <button type="submit"
                            class="block w-full text-center py-3 px-3 rounded-md text-red-500 bg-red-900/40 hover:bg-red-900/60 hover:text-red-400 transition-colors duration-300">Logout</button>
                    </form>
                @endauth
            </div>
        </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        window.addEventListener('click', function(event) {
            if (productMenuDropdown && !productMenuButton.contains(event.target) && !productMenuDropdown
                .contains(event.target)) {
                productMenuDropdown.classList.add('hidden');
            }
            if (userMenuDropdown && !userMenuButton.contains(event.target) && !userMenuDropdown
                .contains(event.target)) {
